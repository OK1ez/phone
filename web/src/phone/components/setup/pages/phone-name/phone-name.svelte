<script lang="ts">
  ...
  let phoneName = $derived(setupApp.phoneName);
</script>

<div class="flex h-full w-full flex-col justify-between px-5 pb-6 pt-16">
  <div class="space-y-4">
    <div class="space-y-0.5">
      <p class="text-base font-medium">Name your device</p>
      <p class="text-xs text-muted-foreground">This name is used for your new account.</p>
    </div>
    ...
    <div class="bg-input/40 hover:bg-input/60 h-8 rounded-lg w-full flex items-center px-3 space-x-2 border">
      <input
        ...
        value={phoneName}
        oninput={(event) => setupApp.setPhoneName(event.currentTarget.value)}
      />
    </div>
  </div>
  ...
  <div class="absolute bottom-6 left-0 w-full flex px-4 space-x-2 z-10">
    ...
    <button
      ...
      onclick={() => setupApp.submitPhoneName()}
      disabled={setupApp.phoneName.trim() === ""}
    >
      Continue
    </button>
  </div>
