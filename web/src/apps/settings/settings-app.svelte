<script lang="ts">
  ...
  const routes: Record<RouteKey, () => Promise<{ default: ComponentType }>> = {
    ...
    notifications: () =>
      import("./pages/notifications/notifications-page.svelte"),
    notificationsOptions: () =>
      import("./pages/notifications/notifications-options-page.svelte"),
  ...
  let CurrentComponent: ComponentType | null = $state(null);
...
<div class="relative flex flex-col w-full h-full bg-background">
  {#key $ACTIVE_PAGE}
    <div
      ...
      in:fly={{
        x: $NAVIGATION_DIRECTION === "forward" ? 500 : -500,
        duration: 300,
      }}
      out:fly={{
        x: $NAVIGATION_DIRECTION === "forward" ? -500 : 500,
