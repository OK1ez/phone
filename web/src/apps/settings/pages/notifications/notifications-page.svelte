...
<ScrollArea class="flex flex-col w-full h-full max-h-[49.5rem] overflow-y-auto">
  {#each Object.entries($APPS) as [appKey, app]}
    <button onclick={() => selectNotificationItem(appKey)} class="flex items-center justify-between w-full h-20 px-6 border-b hover:bg-secondary/70 dark:hover:bg-secondary/20">
      <div class="text-left">
